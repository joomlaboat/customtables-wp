	...
	protected function renderPublishIcon(): string
	{
		if ($this->isPublishable) {
			...
			if ($this->row['listing_published']) {
				...
				$icon = Icons::iconPublished($this->ct->Env->toolbarIcons, esc_html__("Unpublish", "customtables"));
			} else {
				$link = 'javascript:ctPublishRecord(' . $this->Table->tableid . ',\'' . $this->listing_id . '\', \'' . $rid . '\',1,' . $moduleIDString . ');';
				$icon = Icons::iconUnpublished($this->ct->Env->toolbarIcons, esc_html__("Publish", "customtables"));
			}
