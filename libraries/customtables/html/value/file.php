...
//security letters tells what method used
	...
	public function display()
	{
		...
		if ($this->key == $test_key) {
			...
			throw new Exception(esc_html__("Download link is expired", "customtables"));
		}
	...
	function render_blob_output($filename)
	{
		...
		if (count($rows) < 1)
			throw new Exception(esc_html__("File not found", "customtables"));
	...
	{
		if (!file_exists($filePath))
			throw new Exception(esc_html__("File not found", "customtables") . ': \'' . $filePath . '\'');
