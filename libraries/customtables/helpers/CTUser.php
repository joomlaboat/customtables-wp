...
class CTUser
{
	...
	public static function CreateUser(string $realtablename, string $realidfieldname, string $email, string $name, string $usergroups, string $listing_id, string $useridfieldname): bool
	{
		...
		if ($realUserId !== null) {
			...
			throw new Exception(esc_html__("New users created. Passwords sent to their emails.", "customtables"));
		} else {
			throw new Exception(esc_html__("This User cannot be created:", "customtables"));
		}
	}
	...
	static public function CreateUserAccount(string $fullName, string $username, string $password, string $email, string $group_names): ?int
	{
		...
		if (!$user->bind($data)) {
			if (defined('_JEXEC')) {
				if (!CUSTOMTABLES_JOOMLA_MIN_4)
					throw new Exception(esc_html__("User registration failed.", "customtables") . ': ' . $user->getError() ?? '');
				else
					throw new Exception(esc_html__("User registration failed.", "customtables") . ': ' . implode(',', $user->getErrors()));
			}
		}
