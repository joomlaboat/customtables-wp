...
class Twig_HTML_Tags
{
	...
	function recordcount(): string
	{
		...
		return '<span class="ctCatalogRecordCount">' . esc_html__("Found", "customtables") . ': ' . $this->ct->Table->recordcount
			. ' ' . esc_html__("Result(s)", "customtables") . '</span>';
	}
...
';
	...
	function limit($the_step = 5, $showLabel = false, $CSS_Class = null): string
	{
		...
		$result = '';
		if ($showLabel)
			$result .= esc_html__("Show", "customtables") . ': ';
	...
	function orderby($listOfFields = null, $showLabel = false, $CSS_Class = null): string
	{
		...
		$result = '';
		if ($showLabel)
			$result .= esc_html__("Order by", "customtables") . ': ';
	...
	protected function getOrderBox(Ordering $ordering, ?string $listOfFields, string $CSS_Class): string
	{
		...
		$fieldsToSort = [
			['value' => '', 'label' => ' - ' . esc_html__("Order by", "customtables")]
		];
	...
	function goback($defaultLabel = null, $image_icon = '', $attribute = '', string $returnto = ''): string //WordPress Ready
	{
		if ($defaultLabel === null)
			$label = esc_html__("Go Back", "customtables");
		else
			$label = $defaultLabel;
		...
		foreach ($buttons as $mode) {
			if ($mode == 'checkbox') {
				...
				if (in_array($mode, $available_modes)) {
					...
					switch ($mode) {
						case 'publish':
							$icons = Icons::iconPublished($this->ct->Env->toolbarIcons, esc_html__("Publish Selected", "customtables"));
							break;
						case 'unpublish':
							$icons = Icons::iconUnpublished($this->ct->Env->toolbarIcons, esc_html__("Unpublish Selected", "customtables"));
							break;
						case 'refresh':
							$icons = Icons::iconRefresh($this->ct->Env->toolbarIcons, esc_html__("Refresh Selected", "customtables"));
							break;
						case 'delete':
							$icons = Icons::iconDelete($this->ct->Env->toolbarIcons, esc_html__("Delete Selected", "customtables"));
							break;
						default:
	...
	function print($linkType = '', $label = '', $class = 'ctEditFormButton btn button-apply btn-primary'): string
	{
		...
		if (empty($label))
			$label = esc_html__("Print", "customtables");
	...
	function search($list_of_fields_string_or_array = null, $class = '', $reload = false, $improved = '', $matchType = "", $stringLength = ""): string
	{
		...
		foreach ($list_of_fields as $field_name_string) {
			if ($field_name_string == '_id') {
				$fld = array(
					...
					'type' => '_id',
					'typeparams' => '',
					'fieldtitle' . $this->ct->Languages->Postfix => esc_html__("Record ID", "customtables"),
				...
				$fld = array(
					...
					'type' => '_published',
					'typeparams' => '',
					'fieldtitle' . $this->ct->Languages->Postfix => esc_html__("Published", "customtables"),
		...
		$field_title = implode(' ' . esc_html__("or", "customtables") . ' ', $fieldTitles);
	...
	protected function getFieldTitles($list_of_fields): array
	{
		...
		foreach ($list_of_fields as $fieldname_string) {
			...
			if ($fieldname == '_id')
				$field_titles[] = esc_html__("Record ID", "customtables");
			elseif ($fieldname == '_published')
				$field_titles[] = esc_html__("Published", "customtables");
			else {
	...
	function searchbutton($label = '', $class_ = ''): string
	{
		...
		$default_Label = esc_html__("Search", "customtables");
	...
	function searchreset($label = '', $class_ = ''): string
	{
		...
		$default_Label = esc_html__("Search Reset", "customtables");
		if ($label == common::ctStripTags($label)) {
			if ($this->ct->Env->toolbarIcons != '') {
	...
	{
		if ($title == '')
			$title = esc_html__("Save", "customtables");
	...
	{
		if ($title == '')
			$title = esc_html__("Save & Close", "customtables");
	...
	{
		if ($title == '')
			$title = esc_html__("Save as Copy", "customtables");
	...
	protected function renderCancelButton($optional_class, $title, $redirectLink, $formName): string
	{
		...
		if ($title == '')
			$title = esc_html__("Cancel", "customtables");
	...
	protected function renderDeleteButton($optional_class, $title, $redirectLink)
	{
		...
		return '<input id="customtables_button_delete" type="button" class="' . $class . '" value="' . $title . '"
				onClick=\'
                if (confirm("' . esc_html__("Do you want to delete?", "customtables") . '"))
                {
