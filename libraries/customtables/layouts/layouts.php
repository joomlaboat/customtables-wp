		...
		if ($this->layoutType == 1 or $this->layoutType == 5) {
			...
			if ($task == 'delete') {
				...
				if (!empty($listing_id)) {
					if ($this->ct->deleteSingleRecord($listing_id) === 1) {
						common::enqueueMessage(__("Record deleted.", "customtables"), 'notice');
					} else {
						common::enqueueMessage(__("Record not deleted.", "customtables"));
					}
				}
			}
			...
			if ($task == 'saveandcontinue' or $task == 'save') {
				...
				if ($record->save($listing_id, false)) {
					common::enqueueMessage(__("Record saved.", "customtables"), 'notice');
				} else {
					common::enqueueMessage(__("Record cannot be saved.", "customtables"));
				}
				...
				common::enqueueMessage(__("Edit canceled.", "customtables"), 'notice');
				$link = common::getReturnToURL();
				if ($link === null)
					$link = $this->ct->Params->returnTo;
				...
				common::redirect($link, __("Edit canceled.", "customtables"));
			}
