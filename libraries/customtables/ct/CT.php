	...
	function getRecord(): bool
	{
		...
		if (!empty($this->Params->recordsTable) and !empty($this->Params->recordsUserIdField) and !empty($this->Params->recordsField)) {
			...
			if (!$this->checkRecordUserJoin($this->Params->recordsTable, $this->Params->recordsUserIdField, $this->Params->recordsField, $this->Params->listing_id)) {
				...
				throw new Exception(esc_html__("Not authorized", "customtables") . ' ONLY USER CREATED THIS RECORD ALLOWED TO EDIT IT.');
			}
		}
		...
		foreach ($statement_items as $item) {
			...
			if (!str_contains($item['equation'], '.')) {
				...
				if (count($statement_parts) != 2)
					throw new Exception(esc_html__("Menu Item - 'UserID Field name' parameter has a syntax error. Error is about '(' character. Correct example: parent(children).user", "customtables"));
				...
				if (!is_object($parent_table_row))
					throw new Exception(esc_html__("Menu Item - 'UserID Field name' parameter has an error: Table not found.", "customtables"));
